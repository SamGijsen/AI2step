...
def generate_observations_twostep(type="drift",T=50,delta=[99999],change_transitions=True,change_rewards=True,seed=0):
    np.random.seed(seed)
    ...
    # Initialize
    for k in range(2):
        p_r[k,0,0] = np.random.uniform(0.25,0.5)
        p_r[k,1,0] = np.random.uniform(0.5,0.75)
                ...
                if change_rewards:
                    for l in range(2):
                        p_r[k,l,t] = sample_trunc_norm(
                                    ...
                                    max=1)
                        if p_r[k,l,t] < 0.25:
                            p_r[k,l,t] = 0.25 + (0.25 - p_r[k,l,t])
                        elif p_r[k,l,t] > 0.75:
                            p_r[k,l,t] = 0.75 - (p_r[k,l,t] - 0.75)
                else:
                    p_r[k,:,t] = p_r[k,:,t-1]
